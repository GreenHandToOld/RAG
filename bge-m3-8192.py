...
"""
主要修改内容： 
1. 修改了模型路径为本地的"./bge-m3"  
2. 在文本前添加"为这个句子生成表示："前缀，这是bge模型的特殊要求  
3. 将向量提取方式从取第一个token改为使用mean pooling  
4. 在batch处理中也添加了相应的修改。
"""
...
def load_model():
    """加载预训练的bge-m3模型和tokenizer"""
    try:
        logging.info("开始加载模型...")
        model_name = "./bge-m3"  # 改为本地模型路径
...
def text_to_vector(text: str, tokenizer, model, device) -> np.ndarray:
    ...
    try:
        ...
        # bge模型使用outputs.last_hidden_state的平均值作为句子表示
        embeddings = outputs.last_hidden_state.mean(dim=1)
        vector = embeddings.cpu().numpy()[0]
...
def process_batch_texts(texts: List[str], tokenizer, model, device, batch_size=4) -> np.ndarray:
    """批量处理文本到向量"""
    vectors = []
    for i in tqdm(range(0, len(texts), batch_size), desc="Processing texts in batches"):
        batch_texts = texts[i:i + batch_size]
        # 为每个文本添加前缀
        batch_texts = ["为这个句子生成表示：" + text for text in batch_texts]
...
def evaluate_model(eval_file, index, vectors, all_texts, description_texts, tokenizer, model, device):
    ...
    try:
        ...
        results_df.to_csv('bge-m3.csv', index=False)
        logging.info("评估结果已保存到 bge-m3.csv")
