...
def load_model():
    """加载预训练的jina-embeddings-v2-base-zh模型和tokenizer"""
    try:
        logging.info("开始加载模型...")
        model_name = "jina-embeddings-v2-base-zh"  # 本地jina模型路径
        ...
        model = AutoModel.from_pretrained(
            model_name,
            attn_implementation="eager",  # 使用eager实现来避免警告
            trust_remote_code=True  # 添加trust_remote_code参数以确保正确加载模型
        ).to(device)
        model.eval()
...
def evaluate_model(eval_file, index, vectors, all_texts, description_texts, tokenizer, model, device):
    ...
    try:
        ...
        results_df.to_csv('jina-embeddings-v2-base-zh.csv', index=False)
        logging.info("评估结果已保存到 jina-embeddings-v2-base-zh.csv")
