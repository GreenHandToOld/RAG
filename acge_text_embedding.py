...
def load_model():
    """加载预训练的acge_text_embedding模型和tokenizer"""
    try:
        logging.info("开始加载模型...")
        model_name = "./acge_text_embedding"  # 改为本地acge模型路径
...
def process_batch_texts(texts: List[str], tokenizer, model, device, batch_size=32) -> np.ndarray:
    ...
    for i in tqdm(range(0, len(texts), batch_size), desc="Processing texts in batches"):
        ...
        inputs = tokenizer(batch_texts, 
                         return_tensors="pt", 
                         max_length=1024,  # 修改这里的max_length
                         truncation=True, 
                         padding=True)
        inputs = {k: v.to(device) for k, v in inputs.items()}
...
def evaluate_model(eval_file, index, vectors, all_texts, description_texts, tokenizer, model, device):
    ...
    try:
        ...
        results_df.to_csv('acge_text_embedding.csv', index=False)
        logging.info("评估结果已保存到 acge_text_embedding.csv")
