...
def load_model():
    """加载预训练的bge-multilingual-gemma2模型和tokenizer"""
    try:
        logging.info("开始加载模型...")
        model_name = "bge-multilingual-gemma2"  # 改为本地bge-multilingual-gemma2模型路径
...
def evaluate_model(eval_file, index, vectors, all_texts, description_texts, tokenizer, model, device):
    ...
    try:
        ...
        results_df.to_csv('bge-multilingual-gemma2.csv', index=False)
        logging.info("评估结果已保存到 bge-multilingual-gemma2.csv")
