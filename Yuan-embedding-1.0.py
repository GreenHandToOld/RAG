...
def load_model():
    """加载预训练的Yuan-embedding-1.0模型和tokenizer"""
    try:
        logging.info("开始加载模型...")
        model_name = "Yuan-embedding-1.0"  # 改为本地Yuan模型路径
...
def evaluate_model(eval_file, index, vectors, all_texts, description_texts, tokenizer, model, device):
    ...
    try:
        ...
        results_df.to_csv('Yuan-embedding-1.0.csv', index=False)
        logging.info("评估结果已保存到 Yuan-embedding-1.0.csv")
