...
def load_rerank_model():
    """加载重排模型bge-multilingual-gemma2"""
    try:
        logging.info("开始加载重排模型...")
        model_name = "bge-multilingual-gemma2"  # 更新为新的模型路径
...
def compute_similarity(query: str, texts: List[str], tokenizer, model, device) -> List[float]:
    """计算查询和文本列表之间的相似度，使用更大的batch_size"""
    try:
        # 将查询转换为向量
        query_inputs = tokenizer(query,
                               return_tensors="pt",
                               max_length=8192,  # 设置最大token长度为8192
                               truncation=True,
                               padding=True)
...
def process_batch_texts(texts: List[str], tokenizer, model, device, batch_size=512) -> np.ndarray:
    """批量处理文本到向量，使用更大的batch_size"""
    vectors = []
    try:
...
def evaluate_model(eval_file, index, vectors, all_texts, description_texts, tokenizer, model, device):
    ...
    try:
        ...
        results_df.to_csv('Chuxin-Embedding_ternary.csv', index=False)  # 更新输出文件名
        logging.info("评估结果已保存到 Chuxin-Embedding_ternary.csv")
