...
def load_model():
    """加载预训练的gte_Qwen1.5-7B-instruct模型和tokenizer"""
    try:
        logging.info("开始加载模型...")
        model_name = "gte_Qwen1.5-7B-instruct"
...
def evaluate_model(eval_file, index, vectors, all_texts, description_texts, tokenizer, model, device):
    ...
    try:
        ...
        results_df.to_csv('gte_Qwen1.5-7B-instruct.csv', index=False)
        logging.info("评估结果已保存到 gte_Qwen1.5-7B-instruct.csv")
