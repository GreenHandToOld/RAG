...
def load_model():
    """加载预训练的zpoint_large_embedding_zh模型和tokenizer"""
    try:
        logging.info("开始加载模型...")
        model_name = "zpoint_large_embedding_zh"  # 改为本地zpoint模型路径
...
def evaluate_model(eval_file, index, vectors, all_texts, description_texts, tokenizer, model, device):
    ...
    try:
        ...
        results_df.to_csv('zpoint_large_embedding_zh.csv', index=False)
        logging.info("评估结果已保存到 zpoint_large_embedding_zh.csv")
