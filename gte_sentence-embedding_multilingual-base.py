...
def load_model():
    """加载预训练的gte_sentence-embedding_multilingual-base模型和tokenizer"""
    try:
        logging.info("开始加载模型...")
        model_name = "gte_sentence-embedding_multilingual-base"  # 本地模型路径
        logging.info(f"尝试加载模型: {model_name}")
        
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        logging.info(f"使用设备: {device}")
        
        # 添加trust_remote_code参数
        ...
        # 添加trust_remote_code和attn_implementation参数
        model = AutoModel.from_pretrained(
            model_name,
            trust_remote_code=True,  # 信任模型的自定义代码
            attn_implementation="eager"  # 使用eager实现来避免其他潜在警告
        ).to(device)
        model.eval()
...
def process_batch_texts(texts: List[str], tokenizer, model, device, batch_size=32) -> np.ndarray:
    ...
    for i in tqdm(range(0, len(texts), batch_size), desc="Processing texts in batches"):
        ...
        inputs = tokenizer(batch_texts, 
                         return_tensors="pt", 
                         max_length=8192,  # 修改为Chuxin模型的max_length
                         truncation=True, 
                         padding=True)
        inputs = {k: v.to(device) for k, v in inputs.items()}
...
def evaluate_model(eval_file, index, vectors, all_texts, description_texts, tokenizer, model, device):
    ...
    try:
        ...
        results_df.to_csv('gte_sentence-embedding_multilingual-base.csv', index=False)
        logging.info("评估结果已保存到 gte_sentence-embedding_multilingual-base.csv")
